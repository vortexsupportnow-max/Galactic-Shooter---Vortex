...
const GameAssets = (function() {
  ...
  function drawAbilityIcon(abilityId, rarity) {
    ...
    const icons = {
      ...
      pesto_genovese: () => { ctx.fillStyle='#4caf2f'; ctx.beginPath(); ctx.arc(12,13,7,0,Math.PI*2); ctx.fill(); [[4,6],[19,7],[6,20],[18,19]].forEach(([x,y])=>{ ctx.beginPath(); ctx.arc(x,y,2,0,Math.PI*2); ctx.fill(); }); ctx.fillStyle='#8fd45a'; ctx.beginPath(); ctx.ellipse(12,11,5,3,-Math.PI/5,0,Math.PI*2); ctx.fill(); },
      pizza_gigante:  () => { ctx.fillStyle='#c98b3a'; ctx.beginPath(); ctx.arc(12,12,10,0,Math.PI*2); ctx.fill(); ctx.fillStyle='#f2c14e'; ctx.beginPath(); ctx.arc(12,12,8,0,Math.PI*2); ctx.fill(); ctx.fillStyle='#c0221c'; [[9,9],[15,10],[11,15],[16,15]].forEach(([x,y])=>{ ctx.beginPath(); ctx.arc(x,y,2,0,Math.PI*2); ctx.fill(); }); ctx.fillStyle='#3f8f26'; ctx.beginPath(); ctx.arc(8,14,1.5,0,Math.PI*2); ctx.fill(); },
